...
export function MirroredYAxisResolver(
  options: StatisticOptions
): LineChartYAxisResolver {
  return {
    resolve: (data: TrackedStat[]) => {
      ...
      const maxValue = Math.abs(
        [...data].sort((a, b) => Math.abs(b.value) - Math.abs(a.value))[0].value
      );
...
export function AbsoluteYAxisResolver(
  options: StatisticOptions
): LineChartYAxisResolver {
  return {
    resolve: (data: TrackedStat[]) => {
...
export function CenteredYAxisResolver(
  options: StatisticOptions
): LineChartYAxisResolver {
  return {
    resolve: (data: TrackedStat[]) => {
...
export function createYAxisDomainResolver(
  options: StatisticOptions
): LineChartYAxisResolver {
